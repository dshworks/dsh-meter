...
import { DEFAULT_PEAK_PROMPT, foldEvent, init, schema, stateSchema, tariffPrompt, view } from './core.js'
...
export function apply(ctx, config) {
  ...
  ctx.inject(['sessionProjections'], (projectionCtx) => {
    const project = state => view(state, config.currency)
    projectionCtx.sessionProjections.register({
      ...
      // `stateSchema` and an OPTIONAL `wire`, and the registry reads the
      // definition field by field without validating it: on the new registry
      // the old spelling leaves `wire` undefined, which is the documented way
      // to ask for a host-only projection, so the value is silently never sent
      // to the browser and the dock line renders nothing. Carrying both keeps
      // one registration correct on either side of that rename; each registry
      // ignores the pair it does not know.
      stateSchema,
      wire: { viewSchema: schema, view: project },
      schema,
      view: project,
    })
  })
