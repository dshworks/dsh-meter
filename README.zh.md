...
| 56 个测试，CI 绿 | `pnpm test` — 折叠逻辑、时段时钟、价目表、省电提示、余额读取，外加生成产物的同步校验 |
...
默认没有。`dsh-meter` 不加工具、不加系统提示、不加消息、不发模型请求，完全不碰请求本身。花费是付钱的人该看的东西，不该占 agent 的上下文。唯一的可选例外是[省电模式](#省电模式电表开口说话了)（`savingMode: true`）：一个说明当前档位的系统提示小节，当它的文本渲染为空时完全不出现。

#### KV 缓存影响

默认无。省电模式下，`meter:tariff` 小节在同一个档位窗口内逐字节不变，因此 prompt 前缀——以及它的缓存——会跨请求保持命中；小节只在每天四次档位切换处变化，切换后的第一个请求会错过前缀缓存，随后再次稳定。
...
- id: dsh-meter
  config:
    ...
    savingMode: true     # 让模型知道它正站在哪个档位里
```
...
| `savingMode` | `false` | 是否贡献一个说明当前档位的系统提示小节，让模型据此行事 |
| `savingPeakPrompt` | 内置提示语 | 高峰窗口运行期间注入的文本；想让小节闭嘴请用 `savingMode: false`，而不是传空文本 |
| `savingOffPeakPrompt` | `''` | 非高峰时段注入的文本；为空（默认）时该小节渲染为空，不花任何 prompt token |

## 省电模式——电表开口说话了

默认关闭，因为一个读数不该自己长进 agent 的上下文窗口。打开 `savingMode: true` 后，电表不再是旁观者：每次组装系统提示时，它都会贡献一个小节 `meter:tariff`，告诉模型**下一次请求**将按哪个档位发出、应该怎么表现。

高峰窗口内，该小节携带 `savingPeakPrompt`——默认是电表自己的提示语：点明高峰时段（UTC 01:00–04:00 与 06:00–10:00）、账单翻倍的事实，并请模型简洁作答、优先使用已有上下文而不是发起新的工具调用、对可以推迟的重活主动提出等窗口结束再做。窗口之外，该小节渲染 `savingOffPeakPrompt`；默认为空，所以没什么可提醒时电表的声音**一分钱 token 都不花**。想让模型在闲时放开手脚？把 `savingOffPeakPrompt` 设成类似"现在是半价时段，可以大方地用 token"的话即可。

三个刻意的设计：

- **档位在组装时读取，与记账折叠用同一个时钟。** 提示与账单永远不会对下一次请求落在边界的哪一侧产生分歧。
- **文本在一个档位窗口内保持不变。** 这里要是放个倒计时，每分钟都会变，为了一句话把整个会话的 prompt 前缀缓存滚掉——这是本插件能想到的最贵的省钱方式。小节只在每天四个边界处翻转（翻转后的第一个请求会错过前缀缓存），然后再次稳定。
- **这是提醒，不是限流。** 模型可以不理它；请求层没有任何强制。要硬上限，那是另一个插件。
...
- **省电模式是提醒，不是限流。** 小节只是请模型节约，请求层没有任何强制，模型可以不理。且小节在档位边界处翻转，翻转后的第一个请求会错过 prompt 前缀缓存。
