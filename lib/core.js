 ...
 * not written their own. Static on purpose: the assembled prompt must be
 * byte-identical inside a tariff window, or every request after a minute rolls
 * the session's cache prefix — a countdown in here would cost far more in
 * cache misses than the sentence saves.
 */
export const DEFAULT_PEAK_PROMPT = 'Saving mode is on, and DeepSeek is in a peak pricing window right now (01:00-04:00 and 06:00-10:00 UTC): every token bills at twice the off-peak rate, and output is the most expensive bucket. Be economical: answer concisely, prefer context you already have over new tool calls, and avoid long outputs, oversized diffs, and speculative searches. If a task is expensive and can wait, say so and offer to run it after the window closes.'
