...
export function localMidnight(epochMs) {
...
/**
 * The tariff of each local hour in a run of days, read at the midpoint of every
 * hour so a half-hour timezone still lands inside the right window.
 *
 * Each cell asks `tariffAt` for that exact instant rather than indexing the
 * published schedule. The schedule grew a day axis when weekends went off-peak
 * — it is a 7x24 grid on Beijing fields now — and the old reading indexed it by
 * UTC hour, which silently yielded a row object or `undefined` for every cell,
 * so no cell ever compared equal to `'peak'` and the strip drew a flat
 * off-peak day forever. Asking the same function that prices the request cannot
 * drift from it, and there is no index to mistranslate.
 *
 * @param {number} from - local midnight of the first day.
 * @param {number} days - how many days to lay out.
 * @returns {string[][]} one row of 24 tariff names per day.
 */
export function localTariffDays(from, days) {
  return Array.from({ length: days }, (_unused, day) =>
    Array.from({ length: 24 }, (_cell, hour) =>
      tariffAt(from + ((day * 24) + hour + 0.5) * 3_600_000)))
}

/** Local midnight of Monday in the week an instant falls in. */
export function localWeekStart(epochMs) {
  const date = new Date(localMidnight(epochMs))
  date.setDate(date.getDate() - ((date.getDay() + 6) % 7))
  return date.getTime()
}

/**
 * How the current schedule spells its peak hours, for prose that must not
 * outlive it: "01:00-04:00 and 06:00-10:00 UTC, Monday to Friday".
 *
 * Read off PEAK_WINDOWS_UTC and tariffSchedule rather than typed, because a
 * sentence that names a fact the table also holds is a sentence that drifts —
 * this repo has already shipped a peak-hours claim that survived the weekend
 * rule by three days, in a sibling project, for exactly that reason.
 */
export function peakHoursPhrase() {
  const pad = h => String(h).padStart(2, '0')
  const windows = PEAK_WINDOWS_UTC.map(([a, b]) => `${pad(a)}:00-${pad(b)}:00`).join(' and ')
  const grid = tariffSchedule()
  const peakDays = grid.filter(day => day.includes('peak')).length
  const days = peakDays === 7 ? 'every day' : peakDays === 5 ? 'Monday to Friday' : `${peakDays} days a week`
  return `${windows} UTC, ${days}`
}

/**
 * The built-in peak-window nudge, used when saving mode is on and the user has
 * not written their own. Static within a tariff window on purpose: the
 * assembled prompt must be byte-identical inside one, or every request after a
 * minute rolls the session's cache prefix — a countdown in here would cost far
 * more in cache misses than the sentence saves. The hours come from the
 * schedule, so they cannot outlive it.
 */
export const DEFAULT_PEAK_PROMPT = `Saving mode is on, and DeepSeek is in a peak pricing window right now (${peakHoursPhrase()}): every token bills at twice the off-peak rate, and output is the most expensive bucket. Be economical: answer concisely, prefer context you already have over new tool calls, and avoid long outputs, oversized diffs, and speculative searches. If a task is expensive and can wait, say so and offer to run it after the window closes.`

/**
 * The system-prompt contribution under saving mode: the configured text for
 * the tariff running at `now`, or '' for silence. Empty sections render to
 * nothing, so the meter costs zero prompt tokens outside a window it speaks
 * in. The tariff comes from the same clock the billing fold uses, so the
 * nudge and the bill never disagree about which side of a boundary the next
 * request lands on.
 * @param {number} now - assembly time, in epoch milliseconds.
 * @param {{mode?: boolean, peak?: string, offpeak?: string}} saving - the plugin's saving-mode config.
 * @returns {string} the section text for this assembly, '' when silent.
 */
export function tariffPrompt(now, saving = {}) {
  if (saving.mode !== true) return ''
  const tariff = tariffAt(now)
  if (tariff === 'flat') return ''
  if (tariff === 'peak') return saving.peak ?? DEFAULT_PEAK_PROMPT
  return saving.offpeak ?? ''
}
...
/**
 * A published rate at the precision DeepSeek publishes it, no padding:
 * `¥0.05`, `¥1.5`, `$0.0028`, `¥27`.
 *
 * Deliberately not {@link formatMoney}. That one pads a computed cost to a
 * fixed width so a column of session totals lines up and a sub-cent figure
 * survives. A rate card is a quotation, not a total: padding `¥1.5` to `¥1.50`
 * next to `¥0.0500` invents precision the vendor never published and makes one
 * column carry two decimal conventions.
 * @param {number} amount - the listed rate per 1M tokens.
 * @param {string} currency - `usd` or `cny`.
 * @returns {string} display string with its symbol.
 */
export function formatRate(amount, currency) {
  return `${CURRENCY_SYMBOL[currency] ?? ''}${String(amount)}`
}
