...
<img src="docs/meter-light.png" alt="输入框下方的计价行与展开的卡片：会话总价、时段表、缓存与输入拆解、账户余额" width="100%">
...
| 缓存命中 / 未命中输入 / 输出，各自的 token 与金额 | 命中价只有未命中的 **1/50**。这一行能看出前缀是否稳定 |
...
| 47 个测试，CI 绿 | `pnpm test` — 折叠逻辑、时段时钟、价目表、余额读取，外加生成产物的同步校验 |
...
| | 缓存命中 | 缓存未命中 | 输出 |
|---|---|---|---|
| **v4-flash** 统一价 *（至 UTC 8/16 16:00）* | $0.0028 / ¥0.02 | $0.14 / ¥1 | $0.28 / ¥2 |
| v4-flash 空闲 | $0.007 / ¥0.05 | $0.22 / ¥1.5 | $0.66 / ¥4.5 |
| v4-flash 高峰 | $0.014 / ¥0.10 | $0.44 / ¥3 | $1.32 / ¥9 |
| **v4-pro** 统一价 *（至 UTC 8/16 16:00）* | $0.003625 / ¥0.025 | $0.435 / ¥3 | $0.87 / ¥6 |
| v4-pro 空闲 | $0.022 / ¥0.15 | $0.66 / ¥4.5 | $1.98 / ¥13.5 |
| v4-pro 高峰 | $0.044 / ¥0.30 | $1.32 / ¥9 | $3.96 / ¥27 |

高峰为 **UTC 01:00–04:00 与 06:00–10:00**（北京时间 09:00–12:00、14:00–18:00）。其余时间都是空闲时段，包括两个窗口之间那两小时。空闲价正好是高峰价的一半——但仍高于它取代的统一价，输出约为 2.4 倍。

来源：<https://api-docs.deepseek.com/quick_start/pricing>，抓取于 2026-08-13。
...
- **价目表是编译进去的。** DeepSeek 会调价；本插件带的是 2026-08-13 的快照，跟进需要发新版本。
